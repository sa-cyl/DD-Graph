...
struct pagerank_kernel : public functional_kernel<float, float> {
    ...
    float initial_value(ddgraph_context &info, vertex_info& myvertex) {
        return 1.0;
    }
    ...
    // "Gather"
    float op_neighborval(ddgraph_context &info, vertex_info& myvertex, vid_t nbid, float nbval) {
        return nbval;
    }
    ...
    // "Apply"
    float compute_vertexvalue(ddgraph_context &ginfo, vertex_info& myvertex, float nbvalsum) {
        assert(ginfo.nvertices > 0);
        return RANDOMRESETPROB / ginfo.nvertices + (1 - RANDOMRESETPROB) * nbvalsum;
    }
    
    // "Scatter
    float value_to_neighbor(ddgraph_context &info, vertex_info& myvertex, vid_t nbid, float myval) {
