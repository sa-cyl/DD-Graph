...
#include "engine/dynamic_graphs/ddgraph_dynamicgraph_engine.hpp"
...
public:
    ...
    /**
      ...
      */
    int grab_adj(ddgraph_vertex<uint32_t, uint32_t> &v) {
        if(is_pivot(v.id())) {            
    ...
    /** 
      ...
      */
    int intersection_size(ddgraph_vertex<uint32_t, uint32_t> &v, vid_t pivot, int start_i) {
...
struct TriangleCountingProgram : public GraphChiProgram<VertexDataType, EdgeDataType> {
     ...
     */
    void update(ddgraph_vertex<VertexDataType, EdgeDataType> &v, ddgraph_context &gcontext) {
        ...
        if (gcontext.iteration % 2 == 0) {
            ...
            for(int i=0; i<v.num_edges(); i++) {
                ddgraph_edge<uint32_t> * e = v.edge(i);
...
int main(int argc, const char ** argv) {
    ...
    ddgraph_dynamicgraph_engine<VertexDataType, EdgeDataType> engine(filename + "_degord",
                                                                      nshards, scheduler, m); 
