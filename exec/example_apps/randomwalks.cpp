        ...
        if (gcontext.iteration == 0) {
            ...
            if (is_source(vertex.id())) {
                for(int i=0; i < walks_per_source(); i++) {
                    ...
                    ddgraph_edge<EdgeDataType> * outedge = vertex.random_outedge();
                    if (outedge != NULL) {
            ...
            for(int i=0; i < vertex.num_inedges(); i++) {
                ddgraph_edge<EdgeDataType> * edge = vertex.inedge(i);
...
int main(int argc, const char ** argv) {
    ...
    ddgraph_engine<VertexDataType, EdgeDataType> engine(filename, 0, scheduler, m);
    if (preexisting_shards) { //todo:
