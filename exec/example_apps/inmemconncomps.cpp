...
struct ConnectedComponentsProgram : public GraphChiProgram<VertexDataType, EdgeDataType> {
    ...
    vid_t neighbor_value(ddgraph_edge<EdgeDataType> * edge) {
        return vertex_values[edge->vertex_id()];
    }
    
    void set_data(ddgraph_vertex<VertexDataType, EdgeDataType> &vertex, vid_t value) {
     ...
     */
    void before_iteration(int iteration, ddgraph_context &ctx) {
        if (iteration == 0) {
     ...
     */
    void after_exec_interval(vid_t window_st, vid_t window_en, ddgraph_context &ginfo) {
    }
