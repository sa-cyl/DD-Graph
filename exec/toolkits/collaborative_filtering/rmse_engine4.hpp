...
struct ValidationRMSEProgram4 : public GraphChiProgram<VertexDataType, EdgeDataType> {
   ...
   */
  void update(ddgraph_vertex<VertexDataType, EdgeDataType> &vertex, ddgraph_context &gcontext) {
  ...
  void before_iteration(int iteration, ddgraph_context & gcontext){
   ...
   */
  void after_iteration(int iteration, ddgraph_context &gcontext) {
    assert(Le > 0);
...
void init_validation_rmse_engine(ddgraph_engine<VertexDataType,EdgeDataType> *& pvalidation_engine, int nshards,float (*prediction_func)(const vertex_data & user, const vertex_data & movie, float rating, double & prediction, void * extra), bool _time_weighting, bool _time_nodes, int _matlab_time_offset){
  metrics * m = new metrics("validation_rmse_engine");
  ddgraph_engine<VertexDataType, EdgeDataType> * engine = new ddgraph_engine<VertexDataType, EdgeDataType>(validation, nshards, false, *m); 
...
template<typename VertexDataType, typename EdgeDataType>
void run_validation4(ddgraph_engine<VertexDataType, EdgeDataType> * pvalidation_engine, ddgraph_context & context){
