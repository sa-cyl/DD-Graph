...
class adjlist_container {
   ...
   */
  int load_edges_into_memory(ddgraph_vertex<uint32_t, uint32_t> &v) {
   ...
   */
  double calc_distance(ddgraph_vertex<uint32_t, uint32_t> &v, vid_t pivot, int distance_metric) {
...
int main(int argc, const char ** argv) {
 ...
 logstream(LOG_INFO) << "M = " << M << std::endl;
  ...
  ddgraph_engine<VertexDataType, EdgeDataType> engine(training, 1, true, m); 
