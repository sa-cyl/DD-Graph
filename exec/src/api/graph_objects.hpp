...
#define VARIABLE_IS_NOT_USED
    ...
    public:
        ...
        ddgraph_edge() {}
        ddgraph_edge(vid_t _vertexid, EdgeDataType * edata_ptr) : vertexid(_vertexid), data_ptr(edata_ptr) {
        }
    ...
    bool eptr_less(const ddgraph_edge<ET> &a, const ddgraph_edge<ET> &b) {
    ...
    static void VARIABLE_IS_NOT_USED remove_edgev(ddgraph_edge<bool> * e);
    static void VARIABLE_IS_NOT_USED remove_edgev(ddgraph_edge<bool> * e) {
        e->set_data(true);
    }
    
    static void VARIABLE_IS_NOT_USED remove_edgev(ddgraph_edge<vid_t> * e);
    static void VARIABLE_IS_NOT_USED remove_edgev(ddgraph_edge<vid_t> * e) {
        e->set_data(0xffffffff);
    }
    
    static void VARIABLE_IS_NOT_USED remove_edgev(ddgraph_edge<int> * e);
    static void VARIABLE_IS_NOT_USED remove_edgev(ddgraph_edge<int> * e) {
        e->set_data(0xffffffff);
    }
    ...
    protected:
        ddgraph_edge<EdgeDataType> * inedges_ptr;
        ddgraph_edge<EdgeDataType> * outedges_ptr;
    ...
    public:
        ...
        internal_ddgraph_vertex() : inc(0), outc(0) {
#ifdef SUPPORT_DELETIONS
        ...
        internal_ddgraph_vertex(vid_t _id, ddgraph_edge<EdgeDataType> * iptr, 
                                ddgraph_edge<EdgeDataType> * optr, 
                                 int indeg, 
                                 int outdeg) : 
        ...
        virtual ~internal_ddgraph_vertex() {}
...
#endif
            if (inedges_ptr != NULL) 
                inedges_ptr[inc] = ddgraph_edge<EdgeDataType>(src, ptr);
        ...
        inline void add_outedge(vid_t dst, EdgeDataType * ptr, bool special_edge) {
            ...
            if (outedges_ptr != NULL) outedges_ptr[i-1] = ddgraph_edge<EdgeDataType>(dst, ptr);
            assert(dst != vertexid);
        }
    ...
    template <typename VertexDataType, typename EdgeDataType >
    class ddgraph_vertex : public internal_ddgraph_vertex<VertexDataType, EdgeDataType> {
        
    public:
        
        ddgraph_vertex() : internal_ddgraph_vertex<VertexDataType, EdgeDataType>() { }
        
        ddgraph_vertex(vid_t _id, 
                        ddgraph_edge<EdgeDataType> * iptr, 
                                 ddgraph_edge<EdgeDataType> * optr, 
                        int indeg, 
                        int outdeg) : 
            internal_ddgraph_vertex<VertexDataType, EdgeDataType>(_id, iptr, optr, indeg, outdeg) {}
        
        virtual ~ddgraph_vertex() {}
        
        /** 
          * Returns ith edge of a vertex, ignoring 
          * edge direction.
          */
        ddgraph_edge<EdgeDataType> * edge(int i) {
            if (i < this->inc) return inedge(i);
            else return outedge(i - this->inc);
        }

        
        ddgraph_edge<EdgeDataType> * inedge(int i) {
            assert(i >= 0 && i < this->inc);
            return &this->inedges_ptr[i];
        }
        
        ddgraph_edge<EdgeDataType> * outedge(int i) {
            assert(i >= 0 && i < this->outc);
            return &this->outedges_ptr[i];
        }        
        
        ddgraph_edge<EdgeDataType> * random_outedge() {
        ...
        void VARIABLE_IS_NOT_USED sort_edges_indirect() {
            ...
            if (this->inc != (this->outedges_ptr - this->inedges_ptr)) {
                // Moving
                memmove(&this->inedges_ptr[this->inc], this->outedges_ptr, this->outc * sizeof(ddgraph_edge<EdgeDataType>));
