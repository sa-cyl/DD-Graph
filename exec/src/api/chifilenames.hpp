...
#define VARIABLE_IS_NOT_USED
    ...
    /*add by cyl */
    template <typename EdgeDataType>
    static std::string filename_block_edata(std::string basefilename, int exec_interval, int index, int p, int inout){//in:inout = 0 out:inout =1 inout=3 size
         int pos = basefilename.find_last_of('/');
         std::string bn(basefilename.substr(pos + 1) );
         std::stringstream ss;
         ss << basefilename;
         ss << ".subgraph.";
         ss << exec_interval;
    ...
    static std::string filename_block_adj(std::string basefilename, int exec_interval, int index, int p){
         int pos = basefilename.find_last_of('/');
         std::string bn(basefilename.substr(pos + 1) );
         std::stringstream ss;
         ss << basefilename;
    ...
    static std::string filename_shard_edata_block_XG(std::string edata_shardname, int blockid, size_t blocksize) {
        std::stringstream ss;
        ss << dirname_shard_edata_block(edata_shardname, blocksize);
        ss << "/";
        ss << blockid;
    ...
    static std::string filename_block_adj(std::string basefilename, int exec_interval, int index, int p, int inout){//in:inout = 0 out:inout =1
    ...
    static std::string filename_config() {
        ...
        if (chi_root != NULL) {
            return std::string(chi_root) + "/conf/ddgraph.cnf";
        } else {
            return "conf/ddgraph.cnf";
        }
    }
    ...
    static std::string filename_config_local() {
        char * chi_root = getenv("GRAPHCHI_ROOT");
        if (chi_root != NULL) {
            return std::string(chi_root) + "/conf/ddgraph.local.cnf";
        } else {
            return "conf/ddgraph.local.cnf";
        }
    }
