...
struct smoketest_program : public functional_kernel<int, int> {
    ...
    int initial_value(ddgraph_context &info, vertex_info& myvertex) {
        return 0;
    }
    ...
    // "Gather"
    int op_neighborval(ddgraph_context &info, vertex_info& myvertex, vid_t nbid, int nbval) {
    ...
    // "Apply"
    int compute_vertexvalue(ddgraph_context &ginfo, vertex_info& myvertex, int nbvalsum) {
        return ginfo.iteration;
    }
    
    // "Scatter
    int value_to_neighbor(ddgraph_context &info, vertex_info& myvertex, vid_t nbid, int myval) {
