...
namespace ddgraph {
    ...
    /* Add X functions By YongLi Cheng. */
    void writetofile(std::string filename, char * tbuf, size_t nbytes, size_t off){
         int fid = open(filename.c_str(), O_WRONLY);
         if (fid < 0) {
               logstream(LOG_ERROR) << "Could not write file: " << filename 
                << " error: " << strerror(errno) << std::endl;
                                
         }
         assert(fid >= 0);

         pwritea(fid, tbuf, nbytes, 0) ;
         close(fid);     
    }


    void readfromfile(std::string filename, char * tbuf, size_t nbytes, size_t off){
         int fid = open(filename.c_str(), O_WRONLY);
         if (fid < 0) {
         ...
         assert(fid >= 0);
         preada(fid, tbuf, nbytes, 0) ;
         close(fid);     
    }

    void writeedatablock(std::string basefilename, char * buf, int exec_interval, int index , int p, int inout,int nbytes){
        std::string fname =  filename_block_edata<float>(basefilename, exec_interval, index, p, inout);
        writetofile(fname, buf, nbytes, 0);
    }

    void readedatablock(std::string basefilename, char * buf, int exec_interval, int index , int p, int inout, int nbytes){
        std::string fname =  filename_block_edata<float>(basefilename, exec_interval, index, p, inout);
        readfromfile(fname, buf, nbytes, 0);
    }
    /* End */
