...
#include "engine/ddgraph_engine.hpp"
...
namespace ddgraph {
    ...
    template <typename VertexDataType, typename EdgeDataType, typename svertex_t = ddgraph_vertex<VertexDataType, EdgeDataType> >
    class ddgraph_dynamicgraph_engine : public ddgraph_engine<VertexDataType, EdgeDataType, svertex_t> {
    public:
        typedef ddgraph_engine<VertexDataType, EdgeDataType>  base_engine;
        typedef edge_buffer_flat<EdgeDataType> edge_buffer; 
        
        ddgraph_dynamicgraph_engine(std::string base_filename, int nshards, bool selective_scheduling, metrics &_m) :
        ddgraph_engine<VertexDataType, EdgeDataType, svertex_t>(base_filename, nshards, selective_scheduling, _m){
        ...
        virtual vid_t determine_next_window(vid_t iinterval, vid_t fromvid, vid_t maxvid, size_t membudget) {
            ...
            for(int i=0; i < max_interval; i++) {
                ...
                memreq += sizeof(svertex_t) + (sizeof(EdgeDataType) + sizeof(vid_t) + 
                                               sizeof(ddgraph_edge<EdgeDataType>))*(outc + inc);
                if (memreq > membudget) {
...
#define BBUF 32000000
    ...
    public:
        std::string get_info_json() {
            ...
            /**
              * FIXME: too much duplicate with ddgraph_engine
              */
            json << "{";
            json << "\"state\" : \"" << state << "\",\n";
